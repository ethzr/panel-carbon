...
const Limit = ({ limit, children }: { limit: string | null; children: React.ReactNode }) => (
    <>
        {children}
        <span className={'ptero-muted'}> / {limit || <>&infin;</>}</span>
    </>
);
...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('ptero-stack', className)}>
            ...
            <StatBlock icon={faMicrochip} title={'CPU Load'} color={getBackgroundColor(stats.cpu, limits.cpu)}>
                {status === 'offline' ? (
                    <span className={'ptero-muted'}>Offline</span>
                ) : (
            ...
            <StatBlock icon={faCloudDownloadAlt} title={'Network (Inbound)'}>
                {status === 'offline' ? <span className={'ptero-muted'}>Offline</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'Network (Outbound)'}>
                {status === 'offline' ? <span className={'ptero-muted'}>Offline</span> : bytesToString(stats.tx)}
            </StatBlock>
        </div>
    );
};
